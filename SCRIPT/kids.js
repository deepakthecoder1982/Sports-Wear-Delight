...
let kidsArray = JSON.parse(localStorage.getItem("kidsProducts"));
console.log(kidsArray);

displayData(kidsArray);
...
genderFilter.addEventListener("change", function (event) {
  ...
  let filteredData = kidsArray.filter((item) => {
...
sortFilter.addEventListener("change", function (event) {
  ...
  let sortedData = kidsArray;
  if (selectedSort === "highToLow") {
    sortedData = kidsArray.sort((a, b) => b.discount_price - a.discount_price);
  } else if (selectedSort === "lowToHigh") {
    sortedData = kidsArray.sort((a, b) => a.discount_price - b.discount_price);
  }
...
function getCurrentPageData() {
  ...
  return kidsArray.slice(startIndex, endIndex);
}
