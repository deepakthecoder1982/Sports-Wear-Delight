...
let menArray = JSON.parse(localStorage.getItem("menProducts"));
console.log(menArray);

displayData(menArray);
...
genderFilter.addEventListener("change", function (event) {
  ...
  let filteredData = menArray.filter((item) => {
...
sortFilter.addEventListener("change", function (event) {
  ...
  let sortedData = menArray;
  if (selectedSort === "highToLow") {
    sortedData = menArray.sort((a, b) => b.discount_price - a.discount_price);
  } else if (selectedSort === "lowToHigh") {
    sortedData = menArray.sort((a, b) => a.discount_price - b.discount_price);
  }
...
function getCurrentPageData() {
  ...
  return menArray.slice(startIndex, endIndex);
}

//Pagination on the  basis of select tag values;
// const paginationSelect = document.getElementById("pagination");
// let productsPerPage = `${paginationSelect.value}`;
